...
def get_robust_roughness(patch):
    """Enhanced feature extraction using Frequency + Spatial logic."""
    # A. Noise & Glare Reduction (Difference of Gaussians)
    # This removes slow lighting changes (glare) and tiny speckle noise
...
for i, y in enumerate(range(0, h - win_size, step)):
    for j, x in enumerate(range(0, w - win_size, step)):
        ...
        rough_map[i, j] = get_robust_roughness(patch)
...
alpha = np.power(alpha, 1.5) # Gamma correction to make the pink 'pop' more on rough areas
